 ...
 * 
 * Class Functionality:
 * 						The main functionality of this class is publishing the data to kafka server i.e., it checks whether the kafka topics existed or not, if not it creates kafka topics and then consuming the data in a file 
 * 						
 * Methods:
...
public class KafkaSecondLayer {
	...
	//Creation of ZkClient object and initialising it by using loadProperties file
	 ZkClient zkClient = new ZkClient(loadProperties.getSecondLayerProperties().getProperty("ZOOKEEPER.CONNECT"),
			 Integer.parseInt(loadProperties.getSecondLayerProperties().getProperty("SESSIONTIMEOUT")), 
			 Integer.parseInt(loadProperties.getSecondLayerProperties().getProperty("CONNECTIONTIMEOUT")), ZKStringSerializer$.MODULE$);
	...
	public void publish(String TOPIC, String Key, String Message) {
		...
		ZkUtils zkutils = new ZkUtils(zkClient, new ZkConnection(loadProperties.getSecondLayerProperties().getProperty("ZOOKEEPER.CONNECT")), true);
		...
		if (!AdminUtils.topicExists(zkutils, TOPIC)) {
			...
			//Creation of topic based on NUMBEROFPARTITIONS and NUMBEROFREPLICATIONS
			kafkaSecondLayerConnectService.createTopic(TOPIC, Integer.parseInt(loadProperties.getSecondLayerProperties().getProperty("NUMBEROFPARTITIONS")), Integer.parseInt(loadProperties.getSecondLayerProperties().getProperty("NUMBEROFREPLICATIONS")));
		}
		//Creation of Properties object
		Properties properties = new Properties();
		properties.put("metadata.broker.list", loadProperties.getSecondLayerProperties().getProperty("METADATA.BROKER.LIST"));
		properties.put("serializer.class", loadProperties.getSecondLayerProperties().getProperty("SERIALIZER.CLASS"));
		properties.put("reconnect.backoff.ms", loadProperties.getSecondLayerProperties().getProperty("RECONNECT.BACKOFF.MS"));
		properties.put("retry.backoff.ms", loadProperties.getSecondLayerProperties().getProperty("RETRY.BACKOFF.MS"));
		properties.put("producer.type", loadProperties.getSecondLayerProperties().getProperty("PRODUCER.TYPE"));
		properties.put("message.send.max.retries", loadProperties.getSecondLayerProperties().getProperty("MESSAGE.SEND.MAX.RETRIES"));
		properties.put("message.max.bytes", loadProperties.getSecondLayerProperties().getProperty("MESSAGE.MAX.BYTES"));
		...
		//catching the exception for org.apache.kafka.common.KafkaException
			...
			commonsKafkaException.printStackTrace(new PrintWriter(log4jStringWriter));
 ...
 */
	public void consume(String TOPIC) {
		...
		//Creation of Properties object
		Properties properties = new Properties();
		properties.put("zookeeper.connect", loadProperties.getSecondLayerProperties().getProperty("ZOOKEEPER.CONNECT"));
		properties.put("group.id", loadProperties.getSecondLayerProperties().getProperty("GROUP.ID"));
		properties.put("enable.auto.commit", loadProperties.getSecondLayerProperties().getProperty("ENABLE.AUTO.COMMIT"));
		properties.put("auto.commit.interval.ms", loadProperties.getSecondLayerProperties().getProperty("AUTO.COMMIT.INTERVAL.MS"));
		properties.put("auto.offset.reset", loadProperties.getSecondLayerProperties().getProperty("AUTO.OFFSET.RESET"));
		properties.put("session.timeout.ms",loadProperties.getSecondLayerProperties().getProperty("SESSION.TIMEOUT.MS"));
		properties.put("key.deserializer", loadProperties.getSecondLayerProperties().getProperty("KEY.DESERIALIZER"));
		properties.put("value.deserializer", loadProperties.getSecondLayerProperties().getProperty("VALUE.DESERIALIZER"));
