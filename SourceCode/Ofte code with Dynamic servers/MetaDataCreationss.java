...
public class MetaDataCreationss {
	...
	//Creating Logger object for MetaDataCreations class
	 static Logger logger = Logger.getLogger(MetaDataCreationss.class.getName());
	...
	public static void main(String[] args) throws Exception {
		try {
			...
			MetaDataCreationss metaDataCreationss=new MetaDataCreationss();
			...
			for (int i = 0; i < args.length; i++) {
				metaDataMap =metaDataCreationss.mapUpdater(i, args, metaDataMap);
			}
...
//			kafkaServerService.setupEmbeddedKafkaServer();
			//Publishing map data
			kafkaMapData.publish(mapTopicName, metaDataMap.get("monitorName"), metaDataMap.toString().substring(1, metaDataMap.toString().lastIndexOf("}")).replace(" ", ""));
